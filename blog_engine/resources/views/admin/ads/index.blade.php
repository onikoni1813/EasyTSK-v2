...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="p-6 rounded-3xl bg-gradient-to-r from-amber-500/10 via-slate-900 to-slate-950 border border-amber-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-1">Manage banner slots, popunder scripts, custom networks, and automated in-content paragraph injection with strict site isolation.</p>
        </div>

        <div class="flex items-center gap-3 flex-wrap">
            @if($allPlacements->count() > 0)
                <!-- Clear / Turn Off All Ads Button -->
                <form action="{{ route('admin.ads.clear-all') }}" method="POST" onsubmit="return confirm('Turn OFF and clear all ad codes for {{ $site->name }}?');">
                    @csrf
                    <button type="submit" class="px-3.5 py-2.5 bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/30 font-bold text-xs rounded-xl transition flex items-center gap-1.5" title="Turn off all ad codes for this site">
                        <span>🧹</span> Clear All
                    </button>
                </form>
            @endif
    ...
    <!-- Quick Preset Add Toolbar -->
    <div class="p-4 rounded-2xl bg-slate-950/70 border border-slate-800/80 space-y-2">
        <div class="flex items-center justify-between text-xs text-slate-400">
            <span class="font-bold text-white flex items-center gap-1.5">⚡ Quick Add Placement Presets:</span>
            <span class="text-[11px] text-slate-500 font-mono">Click a slot to instantly create an ad unit</span>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            @foreach($slots as $sKey => $sLabel)
                @php
                    $isAlreadyAdded = $allPlacements->contains('placement_slot', $sKey);
                @endphp
                <button type="button" onclick="openNewAdModalWithPreset('{{ $sKey }}', '{{ addslashes($sLabel) }}')" class="px-2.5 py-1.5 {{ $isAlreadyAdded ? 'bg-slate-900 text-slate-400 border-slate-800 hover:text-white' : 'bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border-amber-500/30' }} border rounded-lg text-xs font-semibold transition flex items-center gap-1">
    ...
    @if($allPlacements->isEmpty())
        <!-- Empty State: No Ads Configured -->
        <div class="p-12 rounded-3xl bg-slate-950/90 border border-dashed border-slate-800 text-center space-y-4">
            <div class="w-14 h-14 mx-auto rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 text-2xl">
                📢
            </div>
            <div class="space-y-1">
                <h4 class="text-base font-bold text-white">No Ad Units Configured for {{ $site->name }}</h4>
                <p class="text-xs text-slate-400 max-w-md mx-auto">This site currently has 0 active ads. You can add Adsterra, Monetag, Google AdSense, or custom scripts using the presets above.</p>
            </div>
            <button type="button" onclick="openNewAdModal()" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg shadow-amber-500/20 transition inline-flex items-center gap-1.5">
                <span>➕</span> Create First Ad Unit
            </button>
        </div>
    @else
        <!-- Main Form for Saving Changes to Existing Ad Units -->
        <form action="{{ route('admin.ads.save') }}" method="POST" id="adPlacementsForm" class="space-y-6">
            @csrf

            <!-- Ad Units Grid (Only existing records in DB) -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($allPlacements as $placement)
                    @php
                        $slotKey = $placement->placement_slot;
                    ...
                    <div id="slot-card-{{ $placement->id }}" class="p-5 rounded-2xl bg-slate-950/90 border {{ $isActive ? 'border-amber-500/50 shadow-lg shadow-amber-500/5' : 'border-slate-800' }} space-y-3.5 transition relative flex flex-col justify-between group">
                        <div>
                            <!-- Slot Header with Switch & Direct Delete Button -->
                            <div class="flex items-center justify-between border-b border-slate-800/80 pb-3 gap-2 flex-wrap">
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    <div class="truncate">
                                        <h4 class="text-xs font-bold text-white truncate">{{ $placement->title ?: ucfirst(str_replace('_', ' ', $slotKey)) }}</h4>
                                ...
                                <div class="flex items-center gap-2.5 flex-shrink-0">
                                    ...
                                    <label class="flex items-center gap-1.5 cursor-pointer text-xs bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-800">
                                        <input type="checkbox" name="ads[{{ $slotKey }}][is_active]" value="1" {{ $isActive ? 'checked' : '' }} class="w-3.5 h-3.5 rounded bg-slate-950 border-slate-800 text-amber-500 focus:ring-amber-500">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                                <div>
                                    ...
                                    <input type="text" name="ads[{{ $slotKey }}][title]" value="{{ $placement->title }}" class="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white focus:border-amber-500 focus:outline-none">
                                </div>
                        ...
                        <div class="text-[10px] text-slate-500 font-mono border-t border-slate-800/60 pt-2 flex items-center justify-between">
                            <div>
                                @if($slotKey === 'in_content_p2')
                                    <span>&bull; Injects after paragraph 2</span>
                                @elseif($slotKey === 'in_content_p5')
                                    <span>&bull; Injects after paragraph 5</span>
                                @elseif($slotKey === 'popunder')
                                    <span>&bull; Full-page popunder script</span>
                                @elseif($slotKey === 'header')
                                    <span>&bull; Top billboard banner</span>
                                @elseif($slotKey === 'sidebar_sticky')
                                    <span>&bull; Floating sticky sidebar</span>
                                @elseif($slotKey === 'sidebar_top')
                                    <span>&bull; Sidebar top banner</span>
                                @else
                            ...
                            <div class="flex items-center gap-2">
                                @if($isCustom)
                                    <span class="px-1.5 py-0.5 rounded text-[9px] bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-bold">Custom</span>
                                @endif

                                <!-- Direct Delete Button for this unit -->
                                <button type="button" onclick="deleteSingleAd({{ $placement->id }}, '{{ addslashes($placement->title ?: $slotKey) }}')" class="px-2 py-1 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded-lg text-xs font-bold transition flex items-center gap-1" title="Delete ad unit completely">
                                    <span>🗑️</span> Delete
                                </button>
                            </div>
                        </div>
    ...
    <div class="max-w-lg w-full bg-slate-900 border-2 border-amber-500/30 rounded-3xl p-6 sm:p-7 space-y-4 shadow-2xl animate-scale-in">
        ...
        <form action="{{ route('admin.ads.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" id="modalAdTitle" required placeholder="e.g. Monetag Sidebar 300x250 Banner" class="w-full px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:border-amber-500 focus:outline-none">
            </div>
            ...
            <div>
                ...
                <textarea name="ad_code" id="modalAdCode" rows="4" placeholder="<script src='//...js'></script> or <div>...</div>" class="w-full px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs font-mono text-amber-300 placeholder-slate-600 focus:border-amber-500 focus:outline-none leading-tight"></textarea>
            </div>
...
<!-- Standalone Form for Single Ad Deletion -->
<form id="standaloneDeleteForm" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
...
function openNewAdModalWithPreset(slotKey, slotLabel) {
    openNewAdModal();
    const slotSelect = document.getElementById('slotSelect');
    if (slotSelect) {
        slotSelect.value = slotKey;
        handleSlotSelection(slotKey);
    }
    const titleInput = document.getElementById('modalAdTitle');
    if (titleInput) {
        titleInput.value = slotLabel;
    }
}
...
function deleteSingleAd(placementId, adTitle) {
    if (confirm('Are you sure you want to permanently delete "' + adTitle + '"?')) {
        const form = document.getElementById('standaloneDeleteForm');
        form.action = '{{ route("admin.ads.index") }}/' + placementId;
        form.submit();
    }
}
</script>
