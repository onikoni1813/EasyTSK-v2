...
<div class="space-y-6 sm:space-y-8">
    <div class="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-slate-200/80 shadow-sm space-y-2 sm:space-y-3">
        <div class="flex items-center gap-2">
            <span class="px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white" style="background-color: var(--brand-color)">Tag Archive</span>
            <span class="text-xs text-slate-400 font-mono">{{ $posts->total() }} Articles</span>
        </div>
        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 font-serif-title">#{{ $tag->name }}</h1>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        @empty
            <div class="col-span-full bg-white rounded-2xl p-8 sm:p-12 text-center border border-slate-200">
                <p class="text-sm text-slate-500">No articles tagged with #{{ $tag->name }} yet.</p>
            </div>
        @endforelse
    </div>
